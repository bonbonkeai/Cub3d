...
/*   By: jdu <marvin@42.fr>                         +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/08/30 13:11:51 by jdu               #+#    #+#             */
/*   Updated: 2025/08/30 15:27:53 by jdu              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (game->ray.side == 0)
		game->temp_wall_x = game->player.y + game->temp_perp_dist \
		* game->ray.ray_dir_y;
	else
		game->temp_wall_x = game->player.x + game->temp_perp_dist \
		* game->ray.ray_dir_x;
...
void calculate_tex_x(t_game *game)
{
    ...
    game->wall_params.tex_x = (int)(game->temp_wall_x * \
		(double)game->wall_params.tex->width);
    should_flip = ((game->ray.side == 0 && game->ray.ray_dir_x < 0) || \
				(game->ray.side == 1 && game->ray.ray_dir_y > 0));
    if (should_flip)
        game->wall_params.tex_x = game->wall_params.tex->width - \
		game->wall_params.tex_x - 1;
...
{
	game->wall_params.step = 1.0 * game->wall_params.tex->height / \
		game->draw.line_height;
	game->wall_params.tex_pos = (game->draw.draw_start - game->win_height \
		/ 2 + game->draw.line_height / 2) * game->wall_params.step;
}
